...
/// <summary>
/// High-performance checker for roots with a middle weak letter (Waw/Yah).
/// Optimization: Packs 2 chars into a uint for O(1) lookup.
/// </summary>
public static class ArabicMiddleWeaks
{
    private static readonly HashSet<uint> wawSet;
    private static readonly HashSet<uint> yahSet;

    static ArabicMiddleWeaks()
    {
        wawSet = EncodeSet(ArabicWeaksData.MidWaw);
        yahSet = EncodeSet(ArabicWeaksData.MidYah);
    }
